<template>
  <SfProductCard :title="getName" :image="getImageUrl" :max-rating="5" :score-rating="getProductRating"
    image-width="100%" image-height="400" :link="getRouterLink" class="sw-product-card" :show-add-to-cart-button="true"
    :is-added-to-cart="isInCart" :is-in-wishlist="isInWishlist" @click:add-to-cart="addToCart"
    @click:wishlist="toggleWishlistItem" data-testid="product-card">
    <template #image>
      <SwImage :src="getImageUrl" :title="getName" :alt="getName" style="cursor: pointer" width="200" height="400"
        @click.native="$router.push(getRouterLink)" />
    </template>
    <template #price>
      <SfPrice v-if="displayVariantsFromPrice" class="sw-product-card-price sf-product-card__price variants-price"
        :regular="`${$t('Variants from')} ${displayVariantsFromPrice}`" />
      <SfPrice v-if="displayFromPrice" class="sw-product-card-price sf-product-card__price from-price"
        :regular="`${$t('From')} ${displayFromPrice}`" />
      <SfPrice v-if="displaySinglePrice" class="sw-product-card-price sf-product-card__price real-price"
