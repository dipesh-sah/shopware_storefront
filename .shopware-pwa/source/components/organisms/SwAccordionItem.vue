<template>
  <div class="sw-accordion-item">
    ...
    <slot name="header" v-bind="{
      ...
      <SwButton :aria-pressed="isOpen.toString()" :aria-expanded="isOpen.toString()" :class="{ 'is-open': isOpen }"
        class="sf-button--pure sw-accordion-item__header" @click="accordionClick">
        {{ header }}
        <!-- @slot here you can add additional information about this item -->
        <slot name="additional-info" />
        <SfChevron tabindex="0" class="sw-accordion-item__chevron" :class="{ 'sf-chevron--right': !isOpen }" />
      </SwButton>
    </slot>
